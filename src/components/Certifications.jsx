...
>
          {CERTS.map(c => (
            <div key={c.name}
              ...
              <div style={{color:'#f090c8', fontSize:'0.9rem', fontWeight:600, marginBottom:'4px', lineHeight:1.4}}>{c.name}</div>
              <div style={{color:'var(--grey)', fontSize:'0.8rem', marginBottom:'4px'}}>{c.org}</div>
              <div style={{color:'#f090c8', fontSize:'0.75rem', fontFamily:'var(--font-mono)'}}>{c.date}</div>
            </div>
          ))}
        </div>
      </div>
